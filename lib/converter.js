...
class Converter {
    ...
    async toFile(options) {
        ...
        const result = config.testType === 'ctrf' ? (0, p3x_xml2json_1.toXml)(ctrfConverter_js_1.CtrfConverter.convert(config), this.xmlParserOptions) : await xmlConverter_js_1.XmlConverter.convert(config);
    ...
    async toString(options) {
        const config = config_js_1.ConfigService.config(options);
        return config.testType === 'ctrf' ? (0, xml_formatter_1.default)((0, p3x_xml2json_1.toXml)(ctrfConverter_js_1.CtrfConverter.convert(config), this.xmlParserOptions), { forceSelfClosingEmptyTag: true }) : await xmlConverter_js_1.XmlConverter.convert(config);
    }
    /**
